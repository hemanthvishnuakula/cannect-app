#!/usr/bin/env python3
"""
Cannect Customer Intelligence - DeepSeek Classifier
Extracts consumer intelligence from cannabis social posts.
"""
...
def classify_post(post: Dict[str, Any]) -> Optional[Dict]:
    """Classify a single post using DeepSeek API."""
    try:
        start_time = time.time()
        
        prompt = CLASSIFICATION_PROMPT.format(
            text=post['text_content'][:2000],  # Limit text length
...
def process_batch(batch_size: int = 50):
    ...
    for post in posts:
        try:
            ...
            if classification:
                ...
                if processed % 10 == 0:
                    logger.info(f'Processed {processed}/{len(posts)} posts')
                
                # Rate limit: ~2 requests per second to stay within limits
                time.sleep(0.5)
            else:
                errors += 1
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='CCI DeepSeek Classifier')
    ...
    if args.stats:
        ...
        while True:
            processed = process_batch(args.batch)
            if processed == 0:
